-- New notifications name only lists that requested this signal, retaining old snapshots.
...
begin
    ...
    where w.user_id=account_id and m.stock_asset_id=asset_id and (
      not w.settings_configured or exists(select 1 from watchlist_member_signals s
        where s.watchlist_id=w.id and s.stock_asset_id=asset_id and s.signal_type=category and s.interval=period and s.direction=side)
    );
    return NEW;
end $$;
